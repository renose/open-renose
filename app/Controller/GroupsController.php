...
class GroupsController extends AppController
{
    ...
    function edit($name)
    {
        ...
        else if ($this->Group->save($this->request->data))
        {
    ...
    {
        if(!$this->request->params['isAjax'] || $do == null)
        {
            $group = $this->Group->findByName($name);
            if($group == null)
                throw new NotFoundException();
        ...
        else
        {
            ...
            $done = false;
            foreach($this->request->data['GroupPermission'] as $id => $groupPermission)
            {
                    ...
                    strcasecmp($groupPermission['action'], $action) == 0)
                {
                    if($do == 'allow')
                    {
                        $this->request->data['GroupPermission'][$id]['type'] = 1;
                        $done = true;
                        break;
                    }
                    ...
                    {
                        $this->request->data['GroupPermission'][$id]['type'] = 0;
                        $done = true;
                        break;
                    }
                }
            ...
            {
                if($do == 'allow')
                    $this->request->data['GroupPermission'][] = array('group_id' => $group['Group']['id'], 'controller' => $controller, 'action' => $action, 'type' => 1);

                 else if($do == 'deny')
                    $this->request->data['GroupPermission'][] = array('group_id' => $group['Group']['id'], 'controller' => $controller, 'action' => $action, 'type' => 0);
            }
